async function client(endpoint, { body, ...customConfig }) {
    ...
    if (body) {
        let configAux = {...config, body:JSON.stringify(body)}
        config = configAux;
    }
...
export const httpGet = async function (endpoint, customConfig = {}) {
    return client(endpoint, { body:null,...customConfig, method: 'GET' })
}
...
export const httpDelete = async function (endpoint, customConfig = {}) {
    return client(endpoint, { body:null, ...customConfig, method: 'DELETE' })
}